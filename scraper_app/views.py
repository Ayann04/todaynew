...
def _driver_from_config() -> webdriver.Chrome:
    ...
    Driver path is read from Django settings or env; falls back to webdriver_manager in dev.
    """
    ...
    # In some environments fonts are missing; ensuring basic fonts helps CAPTCHA render
    ...
    if driver_path and os.path.exists(driver_path):
        ...
        if ChromeDriverManager is None:
            # Standard Service() without path assumes chromedriver is in PATH
            logger.warning("CHROMEDRIVER_PATH not provided; using Service() with PATH")
            service = Service()
        else:
            # Fallback: install for local/dev
            logger.info("Installing ChromeDriver via webdriver_manager for local/dev use")
